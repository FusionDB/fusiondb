...
public class MysqlChannel {
    ...
    private static final Logger logger = Logger.get(StmtExecutor.class);
    ...
    public void close() {
        try {
            ...
            logger.warn("Close channel exception, ignore.");
        }
    }
    ...
    public ByteBuffer fetchOnePacket() throws IOException {
        ...
        while (true) {
            ...
            if (readLen != PACKET_HEADER_LEN) {
                ...
                logger.info("Receive packet header failed, remote may close the channel.");
                return null;
            }
            if (packetId() != sequenceId) {
                logger.warn("receive packet sequence id[" + packetId() + "] want to get[" + sequenceId + "]");
            ...
            if (readLen != packetLen) {
                logger.warn("Length of received packet content(" + readLen
