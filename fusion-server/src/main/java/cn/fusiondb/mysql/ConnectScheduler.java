...
// when request come, we new a thread to serve it.
public class ConnectScheduler {
    private static final Logger logger = Logger.get(ConnectScheduler.class);
...
//    }
    ...
    public boolean submit(ConnectContext context) {
        ...
        if (executor.submit(new LoopHandler(context)) == null) {
            logger.warn("Submit one thread failed.");
            return false;
        }
        return true;
    }
    ...
    private class LoopHandler implements Runnable {
        ...
        public void run() {
            try {
                ...
                logger.warn("connect processor exception because ", e);
            } finally {
